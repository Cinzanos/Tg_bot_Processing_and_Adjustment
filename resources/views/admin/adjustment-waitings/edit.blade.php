        ...
        <form action="{{ route('admin.adjustment-waitings.update', $adjustmentWaiting) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="shift_id" id="shift_id" class="mt-1 block w-full border rounded p-2">
                    ...
                    @foreach ($shifts as $shift)
                        <option value="{{ $shift->id }}" {{ old('shift_id', $adjustmentWaiting->shift_id) == $shift->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="start_time" id="start_time" class="mt-1 block w-full border rounded p-2"
                       value="{{ old('start_time', \Carbon\Carbon::parse($adjustmentWaiting->start_time)->format('Y-m-d\TH:i')) }}">
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="end_time" id="end_time" class="mt-1 block w-full border rounded p-2"
                       value="{{ old('end_time', $adjustmentWaiting->end_time ? \Carbon\Carbon::parse($adjustmentWaiting->end_time)->format('Y-m-d\TH:i') : '') }}">
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const currentSectionId = '{{ old('section_id', optional($adjustmentWaiting->equipment->section)->id) }}';
            const currentEquipmentId = '{{ old('equipment_id', $adjustmentWaiting->equipment_id) }}';
