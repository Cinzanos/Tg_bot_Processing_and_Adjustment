...
@section('content')
    <div class="bg-white p-6 rounded shadow">
        <h1 class="text-2xl font-bold mb-4">Редактировать ожидание наладки</h1>
        <form action="{{ route('admin.adjustment-waitings.update', $adjustmentWaiting) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <select name="equipment_id" id="equipment_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($equipment as $item)
                        <option value="{{ $item->id }}" {{ old('equipment_id', $adjustmentWaiting->equipment_id) == $item->id ? 'selected' : '' }}>{{ $item->section }} / {{ $item->machine_number }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="shift_id" id="shift_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($shifts as $shift)
                        <option value="{{ $shift->id }}" {{ old('shift_id', $adjustmentWaiting->shift_id) == $shift->id ? 'selected' : '' }}>{{ $shift->shift_number }} ({{ $shift->date }})</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label for="start_time" class="block text-sm font-medium text-gray-700">Время начала</label>
                <input type="datetime-local" name="start_time" id="start_time" class="mt-1 block w-full border rounded p-2" value="{{ old('start_time', $adjustmentWaiting->start_time) }}">
            </div>
            <div class="mb-4">
                <label for="end_time" class="block text-sm font-medium text-gray-700">Время завершения</label>
                <input type="datetime-local" name="end_time" id="end_time" class="mt-1 block w-full border rounded p-2" value="{{ old('end_time', $adjustmentWaiting->end_time) }}">
            </div>
            <div class="flex space-x-2">
                ...
                <a href="{{ route('admin.adjustment-waitings.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 flex items-center">
