...
@section('content')
    <div class="bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('admin.remarks.update', $remark) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <select name="user_id" id="user_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id', $remark->user_id) == $user->id ? 'selected' : '' }}>{{ $user->full_name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="equipment_id" id="equipment_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($equipment as $item)
                        <option value="{{ $item->id }}" {{ old('equipment_id', $remark->equipment_id) == $item->id ? 'selected' : '' }}>{{ $item->section }} / {{ $item->machine_number }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="shift_id" id="shift_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($shifts as $shift)
                        <option value="{{ $shift->id }}" {{ old('shift_id', $remark->shift_id) == $shift->id ? 'selected' : '' }}>{{ $shift->shift_number }} ({{ $shift->date }})</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="photo" id="photo" class="mt-1 block w-full border rounded p-2" value="{{ old('photo', $remark->photo) }}">
            </div>
            <div class="mb-4">
                ...
                <select name="type" id="type" class="mt-1 block w-full border rounded p-2">
                    @foreach ($types as $type)
                        <option value="{{ $type }}" {{ old('type', $remark->type) == $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            </div>
