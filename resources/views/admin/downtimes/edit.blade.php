            ...
            <div class="mb-4">
                ...
                <select name="user_id" id="user_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id', $downtime->user_id) == $user->id ? 'selected' : '' }}>{{ $user->full_name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="equipment_id" id="equipment_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($equipment as $item)
                        <option value="{{ $item->id }}" {{ old('equipment_id', $downtime->equipment_id) == $item->id ? 'selected' : '' }}>{{ $item->section }} / {{ $item->machine_number }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="shift_id" id="shift_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($shifts as $shift)
                        <option value="{{ $shift->id }}" {{ old('shift_id', $downtime->shift_id) == $shift->id ? 'selected' : '' }}>{{ $shift->shift_number }} ({{ $shift->date }})</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label for="start_time" class="block text-sm font-medium text-gray-700">Время начала</label>
                <input type="datetime-local" name="start_time" id="start_time" class="mt-1 block w-full border rounded p-2" value="{{ old('start_time', $downtime->start_time) }}">
            </div>
            <div class="mb-4">
                <label for="end_time" class="block text-sm font-medium text-gray-700">Время завершения</label>
                <input type="datetime-local" name="end_time" id="end_time" class="mt-1 block w-full border rounded p-2" value="{{ old('end_time', $downtime->end_time) }}">
            </div>
            <div class="mb-4">
