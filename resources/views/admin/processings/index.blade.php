...
@section('content')
    <div class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            <form method="GET" action="{{ route('admin.processings.index') }}" class="flex space-x-4">
                <!-- Поиск по ФИО оператора -->
                <div>
                    <label for="operator_name" class="block text-sm font-medium text-gray-700">ФИО оператора</label>
        ...
        <table class="w-full border-collapse">
            ...
            @foreach ($processings as $processing)
                <tr class="border-t hover:bg-gray-100">
                    <td class="border p-2">{{ $processing->user->full_name }}</td>
                    <td class="border p-2">{{ $processing->equipment->section->name ?? '-' }}</td>
                    <td class="border p-2">{{ $processing->equipment->machine_number }}</td>
                    <td class="border p-2">{{ $processing->shift->shift_number }}</td>
                    <td class="border p-2">{{ $processing->start_time }}</td>
                    <td class="border p-2">{{ $processing->end_time ?? '-' }}</td>
                    <td class="border p-2">{{ $processing->duration ?? '-' }}</td>
        ...
        <div class="mt-4 flex justify-center">
            {{ $processings->appends(request()->query())->links('vendor.pagination.tailwind') }}
        </div>
    </div>
@endsection
