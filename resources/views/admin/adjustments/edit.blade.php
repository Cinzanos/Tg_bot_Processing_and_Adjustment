        ...
        <form action="{{ route('admin.adjustments.update', $adjustment) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="user_id" id="user_id" class="mt-1 block w-full border rounded p-2">
                    <option value="">Выберите наладчика</option>
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id', $adjustment->user_id) == $user->id ? 'selected' : '' }}>
                            {{ $user->full_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="shift_id" id="shift_id" class="mt-1 block w-full border rounded p-2">
                    ...
                    @foreach ($shifts as $shift)
                        <option value="{{ $shift->id }}" {{ old('shift_id', $adjustment->shift_id) == $shift->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="start_time" id="start_time" class="mt-1 block w-full border rounded p-2"
                       value="{{ old('start_time', \Carbon\Carbon::parse($adjustment->start_time)->format('Y-m-d\TH:i')) }}">
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="end_time" id="end_time" class="mt-1 block w-full border rounded p-2"
                       value="{{ old('end_time', $adjustment->end_time ? \Carbon\Carbon::parse($adjustment->end_time)->format('Y-m-d\TH:i') : '') }}">
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const currentSectionId = '{{ old('section_id', optional($adjustment->equipment->section)->id) }}';
            const currentEquipmentId = '{{ old('equipment_id', $adjustment->equipment_id) }}';
