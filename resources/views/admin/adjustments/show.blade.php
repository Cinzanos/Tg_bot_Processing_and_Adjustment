...
@section('content')
    <div class="bg-white p-6 rounded shadow">
        <h1 class="text-2xl font-bold mb-4">Наладка #{{ $adjustment->id }}</h1>
        <p><strong>Наладчик:</strong> {{ $adjustment->user->full_name }}</p>
        <p><strong>Оборудование:</strong> {{ $adjustment->equipment->section }} / {{ $adjustment->equipment->machine_number }}</p>
        ...
        <div class="flex space-x-2">
            <a href="{{ route('admin.adjustments.edit', $adjustment) }}" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 flex items-center">
